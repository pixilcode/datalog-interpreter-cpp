...
namespace parser {
    ...
    Result<vector<Predicate>> predicateList(const ParseInput &input, vector<Predicate> predicatesList) {
        ...
        if (!commaResult.has_value())
            if (predicatesList.empty())
                throw Error("Must have at least one predicate", predicatesResult.first.currToken());
    ...
    Result<vector<Parameter>> parameterList(const ParseInput &input, vector<Parameter> parametersList) {
        ...
        if (!commaResult.has_value())
            if (parametersList.empty())
                throw Error("Must have at least one parameter", parametersResult.first.currToken());
