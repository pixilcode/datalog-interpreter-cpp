...
AutomatonResult QueriesAutomaton::s0(const string &input, int currIndex, int currLine) {
    if (currIndex + 7 <= input.length() && input.substr(currIndex, currIndex + 7) == "Queries") {
        auto token = Token(TokenType::QUERIES, input.substr(currIndex, currIndex + 7), currLine);
...
TestResult QueriesAutomaton::testAutomaton() {
    ...
    auto resultD = queries.start("Queries:", 0, 1);
