...
TestResult ColonAutomaton::testAutomaton() {
    ...
    auto correctValueTests = test::all({
                                               test::assert(resultAValue.finalIndex == 1,
                                                            "colon - didn't advance index"),
                                               test::assert(resultAValue.finalLine == 1,
                                                            "colon - incorrect line number"),
                                               test::assert(resultAToken.type == TokenType::COLON,
                                                            "colon - incorrect token type"),
                                               ...
                                               test::assert(resultDValue.finalIndex == 1,
                                                            "colon - didn't advance index (3)"),
                                               test::assert(resultDValue.finalLine == 1,
                                                            "colon - incorrect line number (3)"),
                                               test::assert(resultDToken.type == TokenType::COLON,
                                                            "colon - incorrect token type (3)"),
