...
TestResult AddAutomaton::testAutomaton() {
    ...
    auto correctValueTests = test::all({
                                               ...
                                               test::assert(resultAToken.type == TokenType::ADD,
                                                            "add - incorrect token type"),
                                               ...
                                               test::assert(resultDValue.finalIndex == 1,
                                                            "add - didn't advance index (3)"),
                                               test::assert(resultDValue.finalLine == 1,
                                                            "add - incorrect line number (3)"),
                                               test::assert(resultDToken.type == TokenType::ADD,
                                                            "add - incorrect token type (3)"),
