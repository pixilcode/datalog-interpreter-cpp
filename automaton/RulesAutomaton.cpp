...
TestResult RulesAutomaton::testAutomaton() {
    ...
    auto correctValueTests = test::all({
                                               test::assert(resultAValue.finalIndex == 5,
                                                            "rules - didn't advance index"),
                                               test::assert(resultAValue.finalLine == 1,
                                                            "rules - incorrect line number"),
                                               test::assert(resultAToken.type == TokenType::RULES,
                                                            "rules - incorrect token type"),
                                               ...
                                               test::assert(resultDValue.finalLine == 1,
                                                            "rules - incorrect line number"),
                                               test::assert(resultDToken.type == TokenType::RULES,
                                                            "rules - incorrect token type"),
