...
AutomatonResult RulesAutomaton::s0(const string &input, int currIndex, int currLine) {
    if (currIndex + 5 <= input.length() && input.substr(currIndex, currIndex + 5) == "Rules") {
        auto token = Token(TokenType::RULES, input.substr(currIndex, currIndex + 5), currLine);
...
TestResult RulesAutomaton::testAutomaton() {
    ...
    auto resultD = rules.start("Rules:", 0, 1);
