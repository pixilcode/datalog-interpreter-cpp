...
AutomatonResult MultiplyAutomaton::s0(const string &input, size_t currIndex, int currLine) {
...
TestResult MultiplyAutomaton::testAutomaton() {
    ...
    auto correctValueTests = test::all({
                                               test::assert(resultAValue.finalIndex == 1,
                                                            "multiply - didn't advance index"),
                                               test::assert(resultAValue.finalLine == 1,
                                                            "multiply - incorrect line number"),
                                               test::assert(resultAToken.type == TokenType::MULTIPLY,
                                                            "multiply - incorrect token type"),
                                               ...
                                               test::assert(resultDValue.finalIndex == 1,
                                                            "multiply - didn't advance index (3)"),
                                               test::assert(resultDValue.finalLine == 1,
                                                            "multiply - incorrect line number (3)"),
                                               test::assert(resultDToken.type == TokenType::MULTIPLY,
                                                            "multiply - incorrect token type (3)"),
                                               test::assert(resultDToken.lexeme == "*",
                                                            "multiply - incorrect lexeme (3)"),
