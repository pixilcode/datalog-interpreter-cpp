...
AutomatonResult CommaAutomaton::s0(const string &input, size_t currIndex, int currLine) {
...
TestResult CommaAutomaton::testAutomaton() {
    ...
    auto correctValueTests = test::all({
                                               test::assert(resultAValue.finalIndex == 1,
                                                            "comma - didn't advance index"),
                                               test::assert(resultAValue.finalLine == 1,
                                                            "comma - incorrect line number"),
                                               test::assert(resultAToken.type == TokenType::COMMA,
                                                            "comma - incorrect token type"),
                                               ...
                                               test::assert(resultDValue.finalIndex == 1,
                                                            "comma - didn't advance index (3)"),
                                               test::assert(resultDValue.finalLine == 1,
                                                            "comma - incorrect line number (3)"),
                                               test::assert(resultDToken.type == TokenType::COMMA,
                                                            "comma - incorrect token type (3)"),
