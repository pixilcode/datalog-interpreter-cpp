...
TestResult ColonDashAutomaton::testAutomaton() {
    ...
    auto correctValueTests = test::all({
                                               test::assert(resultValue.finalIndex == 2,
                                                            "colondash - didn't advance index"),
                                               test::assert(resultValue.finalLine == 1,
                                                            "colondash - incorrect line number"),
                                               test::assert(resultToken.type == TokenType::COLON_DASH,
                                                            "colondash - incorrect token type"),
